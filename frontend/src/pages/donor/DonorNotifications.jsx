                  ...
                  notifications.map((note, idx) => (
                    <div key={note._id || idx} className="notification-card">
                      ...
                      <div>
                        ...
                        {note.actionRequired && (
                          <NotificationActions requestId={note.requestId} />
                        )}
                      </div>
                    </div>
                  ))
...
function NotificationActions({ requestId }) {
  const [responded, setResponded] = React.useState(null); // 'accepted' | 'ignored' | null
  const [loading, setLoading] = React.useState(false);
  const handleRespond = async (response) => {
    setLoading(true);
    await fetch('/api/donor-request/respond', {
      method: 'POST',
  ...
  if (responded === 'accepted') {
    return <div style={{marginTop:'0.7rem',color:'#22c55e',fontWeight:'bold'}}>Accepted! Hospital team notified.</div>;
  }
  if (responded === 'ignored') {
    return <div style={{marginTop:'0.7rem',color:'#ef4444',fontWeight:'bold'}}>Ignored! Hospital team notified.</div>;
  }
  return (
    <div style={{marginTop:'0.7rem'}}>
      <button style={{background:'#22c55e',color:'#fff',border:'none',borderRadius:'0.7rem',padding:'0.4rem 1rem',fontWeight:'bold',marginRight:'0.7rem',cursor:loading?'not-allowed':'pointer'}} disabled={loading} onClick={()=>handleRespond('accepted')}>Accept</button>
      <button style={{background:'#ef4444',color:'#fff',border:'none',borderRadius:'0.7rem',padding:'0.4rem 1rem',fontWeight:'bold',cursor:loading?'not-allowed':'pointer'}} disabled={loading} onClick={()=>handleRespond('ignored')}>Ignore</button>
    </div>
  );
}
