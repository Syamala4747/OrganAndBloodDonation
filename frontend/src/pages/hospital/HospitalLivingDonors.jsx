...
const HospitalLivingDonors = () => {
  ...
  const [requestStatus, setRequestStatus] = useState({}); // { donorId: 'requested' | 'none' }
  ...
  useEffect(() => {
    ...
    async function fetchRequestStatus() {
      try {
        ...
        const res = await fetch('/api/donor-request/hospital', {
  ...
  // Add filteredDonors for search
  const filteredDonors = donors.filter(donor =>
    donor.name?.toLowerCase().includes(search.toLowerCase())
  );

  // Handle donor request and show popup
  const handleRequest = async (donor, idx) => {
    try {
      // Get selected organ from dropdown
      const organSelect = document.getElementById(`organ-select-${donor._id || idx}`);
      const selectedOrgan = organSelect ? organSelect.value : '';
      const token = localStorage.getItem('token');
  const res = await fetch('/api/donor-request/create', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          ...(token ? { 'Authorization': `Bearer ${token}` } : {})
        },
        body: JSON.stringify({ donorId: donor._id, organ: selectedOrgan })
      });
      if (!res.ok) throw new Error('Failed to send donor request');
      setRequestStatus(prev => ({ ...prev, [donor._id]: 'requested' }));
      setPopup({ show: true, message: `Request sent for ${donor.name}.` });
    } catch (err) {
      setPopup({ show: true, message: 'Failed to send request.' });
    }
  };
      ...
      {popup.show && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ marginBottom: '1rem' }}>{popup.message}</div>
            <button onClick={() => setPopup({ show: false, message: '' })} style={{ padding: '0.5rem 1.5rem', borderRadius: '4px', background: '#1976d2', color: '#fff', border: 'none', cursor: 'pointer' }}>OK</button>
          </div>
        </div>
      )}
                  ...
                  filteredDonors.map((donor, idx) => (
                    <div className="living-donor-card" key={donor._id || idx} style={{background:'#fff', borderRadius:'1.2rem', boxShadow:'0 2px 12px #e0e7ef', padding:'1.2rem', marginBottom:'1.2rem', maxWidth:'480px', width:'100%', display:'flex', flexDirection:'row', alignItems:'center', gap:'1.2rem', position:'relative', boxSizing:'border-box', justifyContent:'center', marginLeft:'auto', marginRight:'auto'}}>
                      ...
                      <div className="living-donor-info" style={{flex:1, display:'flex', flexDirection:'column', gap:'0.4rem', justifyContent:'center'}}>
                        ...
                        <div style={{fontSize:'0.97rem'}}><b>Email:</b> {donor.email ? donor.email : <span style={{color:'#f87171'}}>Not provided</span>}</div>
                        ...
                        {Array.isArray(donor.organs) && donor.organs.length > 0 ? (
                          <div style={{marginTop:'0.7rem', display:'flex', alignItems:'center', gap:'0.5rem'}}>
                            ...
                            {requestStatus[donor._id] === 'requested' ? (
                              <button
                                style={{background:'#e5e7eb',color:'#222',border:'none',borderRadius:'0.5rem',padding:'0.08rem 0.4rem',fontWeight:'bold',fontSize:'0.92rem',boxShadow:'0 1px 4px #e0e7ef',cursor:'not-allowed'}}
                                disabled
                              >Requested</button>
                            ) : (
                              <button
